...
def circleDetector(image):
    ...
    ret, img_thresh = cv2.threshold(
        img_edges, 254, 255, cv2.THRESH_BINARY
...
while True:
    ...
    if len(centers) > 0:
        cv2.circle(
            frame, (int(centers[0][0]), int(centers[0][1])), 10, (0, 191, 255), 2
        )  # draw circle[0]
        cv2.putText(
            frame,
            "Measured Position",
        ...
        cv2.putText(
            frame, "Predicted Position", (int(x + 15), int(y)), 0, 0.5, (255, 0, 0), 2
        )
        # Update
